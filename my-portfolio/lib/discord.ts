...
export async function sendVisitorNotification(
  ...
  try {
    ...
    const embed = {
      ...
      footer: { text: `Fingerprint: ${visitorData.fingerprint.substring(0, 12)}...` }
    };

    if (geoData?.is_vpn) {
      embed.fields.push({ name: '⚠️ VPN/Proxy', value: 'Detected', inline: true });
    }
    ...
    if (visitorData.utm_source) {
      ...
      if (utmInfo) {
        embed.fields.push({ name: '🎯 Campaign', value: utmInfo, inline: false });
      }
    }
